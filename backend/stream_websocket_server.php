<?php
/**
 * Servidor WebSocket usando Streams de PHP
...
class StreamWebSocketServer {
    ...
    public function start() {
        // Crear socket usando streams
        $context = stream_context_create();
        $this->socket = stream_socket_server("tcp://0.0.0.0:{$this->port}", $errno, $errstr, STREAM_SERVER_BIND | STREAM_SERVER_LISTEN, $context);
        ...
        while (true) {
            ...
            if (stream_select($read, $write, $except, 0, 200000) > 0) {
                ...
                foreach ($read as $clientStream) {
                    $this->handleClientMessage($clientStream);
                }
            }
        }
    }
    ...
    private function handleNewConnection() {
        $clientStream = stream_socket_accept($this->socket);
        if ($clientStream) {
            $clientId = uniqid();
    ...
    private function handleClientMessage($clientStream) {
        $clientId = $this->findClientId($clientStream);
        if (!$clientId) return;
    ...
    private function findClientId($stream) {
    ...
    private function performHandshake($clientId, $data) {
        ...
        if (isset($headers['Sec-WebSocket-Key'])) {
            ...
            fwrite($this->clients[$clientId]['stream'], $response);
    ...
    private function sendMessage($clientId, $data) {
        ...
        $result = fwrite($this->clients[$clientId]['stream'], $frame);
    ...
    private function disconnectClient($clientId) {
        if (isset($this->clients[$clientId])) {
            fclose($this->clients[$clientId]['stream']);
...
// Iniciar el servidor
$server = new StreamWebSocketServer(8081);
$server->start();
?>